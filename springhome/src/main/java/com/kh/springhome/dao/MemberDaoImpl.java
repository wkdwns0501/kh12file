...
//						+ "order by member_id asc"
			...
			String sql = "select * from ("
					+ "	select rownum rn, TMP.* from ("
					+ "		select * from member where member_level !='관리자' "
					+ "		order by member_id asc"
					+ "	)TMP"
					+ ") where rn between ? and ?";
	...
	public boolean updateMemberInfoByAdmin(MemberDto memberDto) {
		...
		Object[] data= {memberDto.getMemberNickname(), memberDto.getMemberContact(), 
...
//						+ "order by member_id asc"
			...
			String sql = "select * from ("
					+ "	select rownum rn, TMP.* from ("
					+ "		select * from member_list where member_level !='관리자' "
					+ "		order by member_id asc"
					+ "	)TMP"
					+ ") where rn between ? and ?";
			Object[] data = {vo.getStartRow(), vo.getFinishRow()};
	...
	public List<StatDto> selectGroupByMemberLevel() {
		String sql = "select member_level name, count(*) cnt from member group by member_level order by cnt desc";
	...
	public Integer findProfile(String memberId) {
		...
		Object[] data = {memberId};
		try {//queryForObject는 1개의 결과가 나오지 않으면 예외가 발생
